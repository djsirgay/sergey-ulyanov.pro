...
export function mountMeter(doc, win) {
  ...
  const weight = doc.getElementById('weight'), unit = doc.getElementById('unit');
  const sharing = mountShareTools(doc, win);
  ...
  function render() {
    ...
    error.textContent = failedField ? t[failedField + 'Error'] : '';
    for (const [field, element] of [['weight', weight], ['unit', unit]]) {
    ...
    sharing.update(result,lang);
  }
  function calculate() {
    const value = calculateDraniki({weight:weight.value, unit:unit.value});
    result = value.ok ? value : null; failedField = value.ok ? null : value.field;
    render();
    if (failedField) ({weight, unit})[failedField].focus();
  }
  function clear() {
    weight.value = ''; unit.value = 'kg'; result = null; failedField = null; render();
  }
